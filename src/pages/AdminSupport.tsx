...
const AdminSupport = () => {
  ...
  const fetchTickets = async () => {
    try {
      // Super admin can see all tickets via RLS policy
      ...
      if (userIds.length > 0) {
        ...
        // Get emails from auth.users via profiles link or use restaurant_name
  ...
  // Sort tickets by status priority then by date
  ...
  if (selectedTicket) {
    return (
      <>
        <Helmet>
          <title>Admin Suporte - {selectedTicket.subject} | Avalia Pro</title>
  ...
  return (
    <>
      <Helmet>
        <title>Admin Suporte | Avalia Pro</title>
        <meta name="robots" content="noindex, nofollow" />
      </Helmet>
